...
LPDIRECTINPUTDEVICE8 dikeyboard2 = NULL;
...
bool DirectInput_Init(HWND hwnd)
{
	...
	//initialize the keyboard2
	dinput->CreateDevice(GUID_SysKeyboard, &dikeyboard2, NULL);
	dikeyboard2->SetDataFormat(&c_dfDIKeyboard);
	dikeyboard2->SetCooperativeLevel(hwnd, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
	dikeyboard2->Acquire();
...
void DirectInput_Update()
{
	...
	//update keyboard2
	dikeyboard2->Poll();
	if (!SUCCEEDED(dikeyboard2->GetDeviceState(256, (LPVOID)&keys2)))
	{
		//keyboard device lost, try to re-acquire
		dikeyboard2->Acquire();
	}
...
bool Key_Right(int key)
{
	return (bool)(keys2[key] & 0x80);
}

bool Key_Left(int key)
{
	return (bool)(keys2[key] & 0x80);
}
