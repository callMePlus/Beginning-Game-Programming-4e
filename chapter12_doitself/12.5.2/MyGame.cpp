...
QUAD *CreateQuad(char *textureFilename)
{
	...
	string new_textureFilename = string(textureFilename);
	size_t comma_index = new_textureFilename.find(".");

	new_textureFilename = new_textureFilename.substr(0, comma_index) + "_" + to_string(texturenum) +"."+ new_textureFilename.substr(comma_index+1,string::npos);
	D3DXCreateTextureFromFile(d3ddev, new_textureFilename.c_str(), &quad->texture);
